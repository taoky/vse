...
class SubtitleExtractor:
    ...
    def run(self):
        ...
        # 打印加载模型信息
        det_label = (os.path.basename(self.model_config.DET_MODEL_PATH)
                     if self.model_config.DET_MODEL_PATH else self.model_config.DET_MODEL_NAME)
        rec_label = (os.path.basename(self.model_config.REC_MODEL_PATH)
                     if self.model_config.REC_MODEL_PATH else self.model_config.REC_MODEL_NAME)
        self.append_output(f"  DET: {det_label}  |  REC: {rec_label}")
        self.append_output('-----------------------------')
        # 打印视频帧提取开始提示
    ...
    def extract_frame_by_vsf(self):
       ...
       """
                ...
                try:
                    ...
                    for rgb_image in rgb_images:
                        ...
                        if total_ms > last_total_ms:
                            frame_no = int(total_ms * self.fps / 1000)
        ...
        if platform.system() == 'Windows':
            ...
            path_vsf = os.environ.get(
                'VSE_VIDEOSUBFINDER_PATH',
                os.path.join(BASE_DIR, 'subfinder', 'linux', 'VideoSubFinderCli.run'))
            if not (os.path.isfile(path_vsf) and os.access(path_vsf, os.X_OK)):
                raise RuntimeError(f'VideoSubFinder is not an executable file: {path_vsf}')
        # ：图像上半部分所占百分比，取值【0-1】
        ...
        if self.hardware_accelerator.has_rocm() or len(self.hardware_accelerator.onnx_providers) > 0:
        ...
        if platform.system() == 'Windows':
            # 定义执行命令
            cmd = build_vsf_command(
                path_vsf, self.video_path, self.temp_output_dir, self.vsf_subtitle,
                top_end, bottom_end, left_end, right_end, cpu_count,
                config.videoSubFinderDecoder.value.value,
                use_cuda=self.hardware_accelerator.has_cuda(),
                include_ocr_threads=True)
            # 计算进度
            try:
            ...
            self.vsf_running = True
            try:
                # The auditable upstream source build does not emit the
                # patched "Frame:" stderr protocol used by the bundled .run.
                # Monitor its documented RGBImages output instead.
                Thread(target=count_process, daemon=True).start()
                p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1,
    ...
    def _timestamp_to_frameno(self, time_ms):
        return int(time_ms * self.fps / 1000)
