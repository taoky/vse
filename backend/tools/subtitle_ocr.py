...
def async_start(video_path, raw_subtitle_path, sub_area, options):
    ...
    # ROCm/CUDA runtimes cannot be safely inherited through Linux fork.
    # Spawn gives the OCR worker a clean accelerator context.
    ctx = multiprocessing.get_context('spawn')
    task_queue = ctx.Queue()
    # 创建一个进度更新队列
    progress_queue = ctx.Queue()
    # 新建一个进程
    p = ctx.Process(target=subtitle_extract_handler,
