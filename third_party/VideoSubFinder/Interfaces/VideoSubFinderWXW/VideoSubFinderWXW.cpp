...
#else
		// Keep the locally-built CLI self-contained.  The upstream implementation
		// passed a path through system(3) to canberra-gtk-play, which unnecessarily
		// adds a shell/external-process boundary for a non-essential notification.
		SaveToReportLog("PlaySound: disabled in the hardened local build\n");
#endif
	}
	else
	{
